...
public class Step05Usingredicate {
...
//		System.out.println(getAllEmployeesFilteredBy(experienceFilter));
		...
		System.out.println(getAllEmployeesFilteredBy(employee -> employee.getUnit() == Unit.FS));
		System.out.println(getAllEmployeesFilteredBy(employee -> employee.getUnit() == Unit.MS));
		System.out.println(getAllEmployeesFilteredBy(employee -> employee.getSkills().contains(Skill.JAVA)));
		System.out.println(getAllEmployeesFilteredBy(employee -> employee.getExperience() >= 10));
	...
	private static List<Employee> getAllEmployeesFilteredBy(Predicate<Employee> filter) {
		...
		for (Employee employee : employees) {
			if (filter.test(employee)) {// 비교로직을 제외하고는 모두 중복 로직
